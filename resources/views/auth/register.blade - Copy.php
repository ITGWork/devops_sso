...
(function(){
  ...
  function unlockScroll(){
    try{
      ...
      // remove common lock classes from modals/Swal2
      ...
      // kill leftover overlay nodes that can eat scroll/touch
      ['.swal2-container','.modal-backdrop','.loadingoverlay','.blockUI'].forEach(function(sel){
        var nodes = document.querySelectorAll(sel);
        for(var i=0;i<nodes.length;i++){ var n=nodes[i]; if(n && n.parentNode) n.parentNode.removeChild(n); }
      });

      // unblock wheel/touch handlers some plugins attach
      if (window.jQuery){
        try{
  ...
  // run once on DOM ready, and one tick later (in case something flips it late)
  domReady.then(function(){ unlockScroll(); requestAnimationFrame(function(){ setTimeout(unlockScroll, 0); }); });

  /* ============================================================
     ADDITIVE HELPER (new): wait until their P2 UI is truly ready,
     then call data_pid(tax). This replaces the old blind timeout.
     ============================================================ */
          ...
          if (P2_CONFIRM.test(title)) {
            const autoPress = () => {
              ...
              if (btn && !btn.disabled && btn.offsetParent !== null) {
                ...
                // wait until UI ready, then fill once
                waitForP2ThenFill(tax);

                // unlock scroll after modal close
                setTimeout(unlockScroll, 0);
                setTimeout(unlockScroll, 300);
  ...
  // ---------- app ready: connection=true or button enabled (unchanged) ----------
  ...
  // ensure iCheck selects & wait until their UI fully resets for P2 (placeholder + maxlength + person_type)
  function setApplicantAndWait(val){
    return new Promise(resolve=>{
      ...
      if (input && val === '2'){ // บุคคลธรรมดา target UI state
        const isReady = () => {
        ...
        if (isReady()) finish();
          ...
          setTimeout(finish, 1500); // safety: allow their iCheck/Select2 chain to complete
        }
      } else {
        // other types: short settle time
        setTimeout(finish, 150);
      }

      // trigger selection the iCheck way
  ...
  function clickSearch(){
    ...
    if (btn){
      ...
      // jQuery fallback (some setups only listen via jQuery)
  ...
  Promise.all([domReady,searchHandlerReady,pluginsReady,appReady]).then(async function(){
    ...
    // 1) select applicant via iCheck and WAIT until their on-ifChanged UI reset is done (esp. P2)
    await setApplicantAndWait(mapped);

    // 2) now type tax (after their handler has reset the field)
    $tax.prop('readOnly',false);
    nativeSet($tax,tax);

    // done: mark boot end and make sure page can scroll
    window.__PREREG_BOOTING = false;
    unlockScroll();
    setTimeout(unlockScroll, 150);  // one extra nudge if something flips overflow late
  });
})();
</script>
@endpush
