            ...
            <div class="white-box">
                ...
                <h3 class="box-title pull-left">
                    ...
                    @if(!empty($currentReportNo))
                        <span class="label label-info" style="margin-left:10px;">Report No: {{ $currentReportNo }}</span>
                    @endif
                    @if(!empty($is_retest) && $is_retest)
                        <span class="label label-warning" style="margin-left:10px;">งานแก้ / ทดสอบซ้ำ</span>
                    @endif
                </h3>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-8"><p class="form-control-static">{{ $licensee }}</p></div>
                                </div>
                                <div class="form-group">
                                    <label class="col-md-4 control-label">ใบอนุญาต :</label>
                                    <div class="col-md-8"><p class="form-control-static">{{ $licenseNo }}</p></div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="col-md-4 control-label">มาตรฐาน :</label>
                                    <div class="col-md-8"><p class="form-control-static">{{ $stdText }}</p></div>
                                </div>
                                <div class="form-group">
                                    <label class="col-md-4 control-label">รายละเอียดสินค้า :</label>
                                    <div class="col-md-8"><p class="form-control-static">{{ $product->product_detail ?? '-' }}</p></div>
                                </div>
                                @if(!empty($lab_user))
                                <div class="form-group">
                                    <label class="col-md-4 control-label">ห้องปฏิบัติการ :</label>
                                    <div class="col-md-8"><p class="form-control-static">{{ $lab_user->name ?? '-' }}</p></div>
                                </div>
                                @endif
                            </div>
                        </div>
                ...
                {{-- ข้อมูลจากห้องปฏิบัติการอื่น (ปิดการแสดงผลชั่วคราว) --}}
                @if(false && $other_reports->count() > 0)
                    <div class="panel panel-info">
                                ...
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ url('section5/product-testing/report/' . $other->product_lab_id . '?report_no=' . $other->report_no) }}"
                ...
                {{-- แบบฟอร์มผลการทดสอบ --}}
                {{ Form::open(['url' => route('product-testing.report-save'), 'class' => 'form-horizontal', 'method' => 'POST', 'files' => true]) }}
                ...
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body">
                        ...
                        @include('section5.product-testing.partials.test_report_table', ['readonly' => false])
                            ...
                            <div class="col-md-6">
                                <div class="form-group required">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <small class="text-muted">* ถ้า "ไม่ผ่านบางรายการ" ระบบจะส่งกลับเป็นงานทดสอบซ้ำ</small>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group {{ empty($report->report_file_path) ? 'required' : '' }}">
                                    <label class="col-md-4 control-label">Full Report (PDF) :
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        @endif
                                        <input type="file" name="full_report_pdf" id="full_report_pdf" class="form-control" accept="application/pdf"
                                            {{ empty($report->report_file_path) ? 'required' : '' }}>
                        ...
                        <div class="form-group">
                            <div class="col-md-12 text-center">
                                <button type="submit" class="btn btn-success btn-lg"
                                    onclick="return validateAndConfirmReport()">
                                    <i class="fa fa-paper-plane"></i> ส่งรายงานผล
                                </button>
                                <a href="{{ url('section5/product-testing') }}" class="btn btn-default btn-lg">ยกเลิก</a>
                            </div>
                        </div>
...
@push('js')
<script>
function validateAndConfirmReport() {
    @if(empty($report->report_file_path))
    var fileInput = document.getElementById('full_report_pdf');
    if (!fileInput || fileInput.files.length === 0) {
        Swal.fire({
            icon: 'warning',
            title: 'กรุณาแนบไฟล์',
            text: 'กรุณาแนบไฟล์ Full Report (PDF) ก่อนบันทึก',
            confirmButtonText: 'ตกลง'
        });
        return false;
    }
    @endif
    return confirm('ยืนยันส่งรายงานผลทดสอบ?');
}
</script>
@endpush
