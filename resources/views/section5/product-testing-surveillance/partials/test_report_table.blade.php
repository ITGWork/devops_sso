...
@php
    // แยกตารางตามรายละเอียดผลิตภัณฑ์ (maplap_id) เพราะคำขอเดียวกันอาจมีหลายรายการทดสอบ (save_example_map_lap หลาย id)
    $allTestItems = $test_items;
    $groupedTestItemsByMaplap = $allTestItems->groupBy(function($x) { return $x->maplap_id ?? 0; });
@endphp

<div id="tbl-test-result">
@foreach($groupedTestItemsByMaplap as $groupMaplapId => $test_items)
    @php
    ...
    @if($groupMaplapId != 0)
        <h4 style="margin-top:20px; margin-bottom:10px;">
            รายละเอียดผลิตภัณฑ์: {{ $test_items->first()->detail_label ?? ('#' . $groupMaplapId) }}
        </h4>
    @endif
        ...
        <tbody>
            @forelse($test_items as $ti)
                ...
                @php
                    ...
                    // ตั้งไว้ ใช้กับช่องกรอกค่าเดียว (integer/decimal) เท่านั้น — พอร์ตมาจากฝั่ง e-license
                    // (product-testing) เตรียมไว้ล่วงหน้า ยัง "ไม่มีผล" ตอนนี้เพราะ _testItemQuery() ของ
                    // surveillance ยังไม่ได้ join ตาราง bcertify (เฟส 2 ที่ยังไม่ทำ — ดูหัวข้อ 10 ของ
                    // docs/tiw/std-test-item-elicense-groups-exploration.md ที่ devops_center)
                    // $ti->bcertify_comparison_type จะเป็น null เสมอตอนนี้ จึงไม่ตัดค่า 7.2 เดิมไปเลย
                    ...
                    // format_result เป็นตัวตัดสินเดียว (ยืนยันกับผู้ใช้แล้ว 2026-09-09 ดูหัวข้อ 12 ของ
                    // docs/tiw/std-test-item-elicense-groups-exploration.md — devops_center) แสดงแค่
                    // ชื่อ/ข้อของรายการ (มีอยู่แล้วเสมอ) ส่วนช่องอื่นที่ต้องพึ่งข้อมูลที่ยังไม่ได้ตั้งให้บอกตรงๆ
                    // ว่ายังไม่ได้ตั้งค่า แทนช่องกรอกที่กรอกอะไรก็ได้แบบไม่มีความหมาย ห้องแล็บยังส่งรายงานผลได้
                    // ตามปกติผ่านการแนบไฟล์ PDF — พอร์ตมาจากฝั่ง e-license (product-testing) ดูหัวข้อ 9 ด้วย
                ...
                <tr class="js-test-row" data-times="{{ $times }}" data-test_item_id="{{ $ti->id }}" data-unconfigured="{{ $isUnconfigured ? '1' : '0' }}">
                    ...
                    <td style="white-space:pre-wrap;">{!! !empty($ti->tool_text) ? e($ti->tool_text) : ($isUnconfigured ? $notSetLabel : ($ti->test_tools_id ?? '-')) !!}</td>

                    {{-- ✅ ฝั่งขวา: แบบ Snippet --}}
                    @for($seq=1;$seq<=$maxTimes;$seq++)
                        @php
                            $maplapIdKey = $ti->maplap_id ?? 0;
                            $old = isset($existing_results[$maplapIdKey][$ti->id][$seq]) ? $existing_results[$maplapIdKey][$ti->id][$seq] : null;
                        ...
                        >
                            ...
                            @else
                                ...
                                @else
                                    ...
                                    @if($type === 'mix')
                                        ...
                                        @foreach($mixList as $k => $sub)
                                            @php
                                                ...
                                                $subSelectedVal = old("measured_mix.$maplapIdKey.$ti->id.$seq.$k", $subOld);
                                            ...
                                            >
                                                @switch($subType)
                                                    ...
                                                    @case('decimal')
                                                        ...
                                                        type="number"
                                                        step="{{ $subStep }}"
                                                        name="measured_mix[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}][{{ $k }}]"
                                                    ...
                                                    @case('decimal_range')
                                                        ...
                                                        <div class="js-range-wrap" style="display:flex; align-items:center; gap:8px; width:100%;">
                                                            <input
                                                                ...
                                                                name="measured_mix[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}][{{ $k }}][start]"
                                                            ...
                                                            <input
                                                                type="number"
                                                                step="{{ $subType === 'decimal_range' ? '0.01' : '1' }}"
                                                                name="measured_mix[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}][{{ $k }}][end]"
                                        ...
                                        @switch($type)
                                            ...
                                            @case('decimal')
                                                <input
                                                    type="number"
                                                    step="{{ $step }}"
                                                    name="measured[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}]"
                                                    class="form-control input-sm js-measured"
                                                    value="{{ old("measured.$maplapIdKey.$ti->id.$seq", $oldMeasured) }}"
                                                    placeholder="ค่า"
                                                    style="width:100%;"
                                                    data-format="{{ $type }}"
                                                    data-min="{{ $min !== null ? $min : '' }}"
                                            ...
                                            @case('decimal_range')
                                                @php
                                                    $rangeOld = old("measured.$maplapIdKey.$ti->id.$seq", $oldMeasured);
                                                ...
                                                <div class="js-range-wrap" style="display:flex; align-items:center; gap:8px; width:100%;">
                                                    <input
                                                        ...
                                                        name="measured[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}][start]"
                                                        class="form-control input-sm js-range-start"
                                                        value="{{ $rangeStartVal }}"
                                                        placeholder="ค่าเริ่ม"
                                                        style="width:calc(50% - 12px);"
                                                    ...
                                                    <input
                                                        type="number"
                                                        step="{{ $type === 'decimal_range' ? '0.01' : '1' }}"
                                                        name="measured[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}][end]"
                                                        class="form-control input-sm js-range-end"
                                                        value="{{ $rangeEndVal }}"
                                                        placeholder="ค่าสิ้นสุด"
                                                        style="width:calc(50% - 12px);"
                                            ...
                                            @case('select')
                                                ...
                                                >
                                                    <option value="">-</option>
                                                    @php $selectedVal = old("measured.$maplapIdKey.$ti->id.$seq", $oldMeasured); @endphp
                                            ...
                                            @case('select_multiple')
                                                @php
                                                    $selectedVal = old("measured.$maplapIdKey.$ti->id.$seq", $oldMeasured);
                                                    $selectedArr = [];
                                    ...
                                    {{-- PASS/FAIL --}}
                                    @php
                                        $selectedResult = old("item_result.$maplapIdKey.$ti->id.$seq", $oldResult ?? 'fail');
                                    @endphp

                                    <select name="item_result[{{ $maplapIdKey }}][{{ $ti->id }}][{{ $seq }}]"
  ...
  function evalRowSummaryByRow(rowEl) {
    ...
    function evalNumeric(inputEl) {
        ...
        if (fmt === 'integer' || fmt === 'decimal') {
            ...
            // เกณฑ์ 7.1 มาทับ (ดู test_items query ใน ProductTestingSurveillanceController::_testItemQuery())
